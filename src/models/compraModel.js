...
class Compra {
  ...
  static async create(data) {
    const result = await db.query(
      ...
      [data.data_pedido, data.data_entrega, data.valor_estimado_total, data.valor_final_total, data.aprovado_por_dono, data.telefone, data.dono_banca_id]
    );
    return result.rows[0];
  }
  ...
  static async update(id, data) {
    const result = await db.query(
      ...
      [data.data_pedido, data.data_entrega, data.valor_estimado_total, data.valor_final_total,  data.aprovado_por_dono, data.telefone, data.dono_banca_id, id]
    );
    return result.rows[0];
  }
